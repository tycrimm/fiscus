...
- [ ] **Step 8** — Recurring obligations / burn tracking (nanny, insurance, car payments, subscriptions). Schema for obligations + matching against Plaid transactions.
  - [x] **8a** — `expected_flows` schema (one table for both recurring + one-off, both directions). Range columns (low/expected/high). Optional `illiquid_asset_id` link for cap calls / fund distributions. Forecast helper projects flows into 30-day window. MCP tools: `add_expected_flow`, `list_expected_flows`, `forecast_flows`, `update_expected_flow`, `archive_expected_flow`. Dashboard `Upcoming · Next 30 Days` section shows projected events with inflow/outflow totals + net.
  - [ ] **8b** — Per-instance overrides (e.g. heavier nanny week, late-year paycheck step-down). Defer until we feel the pain.
  - [ ] **8c** — Reconcile `expected_flows` against Plaid transactions (depends on step 5c). Self-calibrating ranges.
